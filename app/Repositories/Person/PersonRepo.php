...
class PersonRepo
{
    ...
    public function authorized($context = null)
    {
        ...
        $query = Person::select(\DB::raw('DISTINCT ' . $this->database_shared . '.uw_persons.*'))
            ->join('roles', $this->database_shared . '.uw_persons.uwnetid', '=', 'roles.uwnetid')
            ->orderBy($this->database_shared . '.uw_persons.lastname')
            ->orderBy($this->database_shared . '.uw_persons.firstname');
        if ($context) {
    ...
    {
        return Person::select($this->database_shared . '.uw_persons.*')
            ->join('roles', function($join) {
                $join->on($this->database_shared . '.uw_persons.uwnetid', '=', 'roles.uwnetid');
            })
    ...
    public function withNoAppointment($paginate = 40)
    {
        return Person::select($this->database_shared . '.uw_persons.*')
            ->leftJoin('edw_appointment_cache', function($join) {
                $join->on($this->database_shared . '.uw_persons.person_id', '=', 'edw_appointment_cache.person_id');
            })
