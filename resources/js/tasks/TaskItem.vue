<template>
    ...
    >
        ...
        <section class="request">
            ...
            <div v-if="!task.isCoenvpay">{{ task.taskSummary }}</div>
            <div v-if="task.description" class="quote">{{ task.description }}</div>
        </section>
        <section class="response">
            <div class="text-sm-bold">{{ task.completedAt }}</div>
            <div v-if="task.isApproved && !task.isCoenvpay">{{ toTitleCase(task.response) }}{{ task.behalf }}</div>
            <div v-if="task.isApproved && task.isCoenvpay">{{ task.taskSummary }}</div>
            <div v-if="task.message" class="quote">{{ task.message }}</div>
            <div v-if="task.isCoenvpay && task.revisitDate && !editRevisit" class="alert alert-warning">
                <div class="pb-3">
            ...
            <task-workday
                class="mt-3"
                v-if="task.isCoenvpay && task.hasWorkdayRole && editRevisit"
                revisitOnly="true"
                @confirmed="(data) => saveTask(data)"
                @closeEdit="closeEdit"
            ></task-workday>
            <button class="btn btn-primary mt-3"
                v-if="task.isApproved && task.isCoenvpay && task.hasWorkdayRole && !editRevisit && !task.revisitDate"
